...
_session: dict = {
    ...
    # Deletion awaiting the user's Confirm/Cancel click: {"token": str, "action": dict}
    # or None. The model can only request a removal; execution happens through
    # /confirm-action, entirely outside the LLM tool loop. The token guards
    # against a stale dialog: one-shot confirms make it possible for an older,
    # unclicked dialog to still be on screen when a newer removal is requested —
    # /confirm-action only executes when the posted token matches the current one.
    "pending_action": None,
}
...
async def info() -> dict:
    ...
    return {
        ...
        "vault": str(_vault),
        "unverified_count": count_unverified_papers(get_store()),
    }
...
async def confirm_action(req: ConfirmActionRequest) -> dict:
    # The human decision point for deletions requested by the model. The
    # token must match the currently pending dialog — a stale dialog left
    # unclicked on screen while a newer removal is requested must not be
    # able to fire against the wrong action.
    pending = _session.get("pending_action")
    if pending is None or pending["token"] != req.token:
    ...
    result = execute_remove(pending["action"], get_store())
...
async def chat(req: ChatRequest) -> StreamingResponse:
    ...
    def run_agent() -> None:
        ...
        def request_confirmation(description: str, action: dict):
            # Store the pending deletion (tagged with a fresh token) and show
            # the dialog; returning None tells the tool the decision is
            # deferred to the human. The token lets /confirm-action reject a
            # stale, previously-shown dialog that gets clicked after a newer
            # one has superseded it.
            token = uuid.uuid4().hex
            _session["pending_action"] = {"token": token, "action": action}
