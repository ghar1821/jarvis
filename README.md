...
A reply you no longer want — the wrong question, a runaway tool loop, a huge
paper being ingested by mistake — can be stopped at any point. While one is
generating, the Send button becomes a red **Stop**; clicking it returns control
straight away, not when the model gets around to noticing.

A stopped turn leaves **no trace**: your question comes back to the input box,
nothing is written to the conversation, and nothing is indexed. You can send
the next message immediately.

The connection is closed too, whichever model you are on, so it really does
stop generating rather than finishing an answer nobody will read. One
exception: a request that has not produced its first word yet — a local model
loading for the first time, typically — keeps going until it does. Your side of
it is over either way.

If it was part-way through adding a document, nothing is written to the
knowledge base at all: an ingest is staged in memory and committed in one go,
so it either lands completely or not at all. The one thing a stop will not
interrupt is that final write (a few seconds at most) — a half-finished
database write is exactly what this avoids.

Anything already generated by a paid model is still billed. Stopping gets your
time back, not the tokens.
