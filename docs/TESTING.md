...
| `test_chat_logging.py` | `vault_chat/chat.py` | A tool wrapper that raises logs the exception (with traceback) to the `vault-chat` logger before returning its short error string, across more than one tool wrapper. An `isolated_log` fixture detaches the module's real `FileHandler` for the test so the run never appends to the user's actual `~/.jarvis/logs/chat.log` |
...
| `digest/daemon.py` (scheduler loop, watchdog observer) | Process-runtime plumbing around APScheduler and watchdog; the decision functions and job bodies they drive are covered by `test_daemon.py` |
